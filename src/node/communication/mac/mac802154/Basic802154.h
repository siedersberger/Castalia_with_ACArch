...
enum MacStates {
	...
	MAC_STATE_CAP = 1002,
	MAC_STATE_GTS = 1003,
	MAC_STATE_LOW_DC = 1004,    // Estado em que o nodo tem baixissimo duty cycle
};
...
class Basic802154: public VirtualMac {
 private:
	...
	int associatedPAN;	// ID of current PAN (-1 if not associated)
    	...
    	// IMPLANTAÇÂO TRABALHO LEONARDO
	
   	//Número máximo de nodes a serem associados ao PAN
   	int maxChild;
   	int nchildren; //guarda o número de nodes associados
	
	//fator multiplicante para o baixissimo duty-cycle
	double low_dc;
	int lostpck;  //limite de pacotes não-recebidos de um nodo; após isso, ele é considerado fora da rede

	//Estrutura do PAN para verificar se nós estão ativos
	map<int,int> lost_association; // <id_nodo, numero de pacotes nao recebidos>
  ...
  protected:
	...
	//My modifications
	public:
		double getBeaconIntervalInSeconds();
		double getSuperframeDurationInSeconds();
};
