...
void Basic802154::startup()
{
	...
	if (printStateTransitions) {
		...
		stateDescr[1004] = "MAC_STATE_LOWDC";
	}

	//SETANDO TIMER PARA IMPRIMIR ENERGIA
	setTimer(ENERGY_PRINT, 0);
	...
	//Limpando estrutura de contagem de pacotes nao recebido dos nos associados
	lost_association.clear();

	//Fator multiplicante do baixissimo duty cycle
	low_dc = par("low_dc");

	//Limite de pacotes que um nodo deixou de enviar para ser considerado fora da rede
	lostpck = par("lostpck");
	...
	switch (index) {
		...
		case FRAME_START: {
			if (isPANCoordinator) {	// as a PAN coordinator, create and broadcast beacon packet
				...
				//Incrementando a estrutura de pacotes nao recebidos - Somente PAN faz isso
				map<int,int>::iterator iter; //iterator para rodar a estrutura association_lost				
				if(nchildren > 0){  //tem filhos
					//rodar map e incrementar 1 para todos
					for (iter = lost_association.begin(); iter != lost_association.end(); iter++) {
						//trace() << "NODO " << iter->first << " tem perdido inicial " << iter->second;
						iter->second = iter->second + 1;
						//trace() << "NODO " << iter->first << " tem perdido final " << iter->second;
					}
					
					//TESTES PARA VERIFICAR SE NODOS SAO REMOVIDOS CORRETAMENTE
					//for (iter = lost_association.begin(); iter != lost_association.end(); iter++) {
					//	trace() << "AQUI: " << iter->first;					
					//}
					//map<int,bool>::iterator iter2;
					//for (iter2 = associatedDevices.begin(); iter2 != associatedDevices.end(); iter2++) {
					//	trace() << "ASSOCIATED DEVICES: " << iter2->first;					
					//}
					//trace() << "Number of child nodes: " << nchildren;

					//Após incrementar a estrutura, já posso fazer a verificação
					//rodar map e verificar se alguem extrapolou o limite de pacotes não-recebidos
					for (iter = lost_association.begin(); iter != lost_association.end(); iter++) {
						if(iter->second > lostpck){ 
							trace() << "Nodo " << iter->first << " sera REMOVIDO por extrapolar o limite de pacotes nao enviados!";
							//removendo nodo da lista de associados
							associatedDevices.erase(iter->first);
							//removendo nodo da lista lost_association
							lost_association.erase(iter);
							//decrementando numero de nodos associados
							nchildren--;
						}
					}
		...
		case LOW_DUTYCYCLE:{
			// Aqui o nodo vai para baixissimo duty cycle. 
			// Quando acordar, deve tentar se associar novamente
			if(associatedPAN == -1){  //realmente nao tem associacao
				trace() << "TENTAR ME ASSOCIAR AGORA";
				setMacState(MAC_STATE_SETUP);
				toRadioLayer(createRadioCommand(SET_STATE, RX));
			} else {
				trace() << "ALGUMA COISA ERRADA!!";
			}
			break;
		}

		case ENERGY_PRINT:{
			trace() << std::setprecision(5) << "ENERGY " << SELF_MAC_ADDRESS << " " << resMgrModule->getSpentEnergy();
			setTimer(ENERGY_PRINT, 600);			
			break;
		}
...
void Basic802154::fromRadioLayer(cPacket * pkt, double rssi, double lqi)
{
	...
	switch (rcvPacket->getMac802154PacketType()) {
		...
		case MAC_802154_BEACON_PACKET: {
			...
			trace() << "ENTREI NO RECEIVED BEACON!!";
		...
		case MAC_802154_ASSOCIATE_PACKET:{
			...
			if (associationRequest_hub(rcvPacket)) {
				...
				//insere o nodo na estrutura de contagem de pacote, com valor inicial de zero!!
				lost_association[rcvPacket->getSrcID()] = 0;
				...
				// Quando negado associação, o PAN responde ao nodo solicitante com a negação (ID do PAN como -1)
				// Ao receber a negação o nodo solicitante deverá entrar em baixissimo duty cycle
				Basic802154Packet *ackPacket = new Basic802154Packet("PAN associate response", MAC_LAYER_PACKET);
				ackPacket->setPANid(-1);
				ackPacket->setMac802154PacketType(MAC_802154_ACK_PACKET);
				ackPacket->setDstID(rcvPacket->getSrcID());
				ackPacket->setByteLength(ACK_PKT_SIZE);
				toRadioLayer(ackPacket);
		...
		case MAC_802154_DATA_PACKET:{
			//preciso verificar se o nodo pertence a lista de associados.
			// Se não pertencer, ele não tem autorização de enviar pacotes de dados
			if(lost_association.find(rcvPacket->getSrcID()) == lost_association.end() ){
				trace() << "Nodo " << rcvPacket->getSrcID() << " nao esta como associado!";
				//avisar o nodo que ele nao tem autorização, para que ele possa atualizar seu status
				Basic802154Packet *ackPacket = new Basic802154Packet("Ack packet", MAC_LAYER_PACKET);
				ackPacket->setPANid(-1);  //setando PAN como -1 para avisar ao nodo que ele nao tem autorização
				ackPacket->setMac802154PacketType(MAC_802154_ACK_PACKET);
				ackPacket->setDstID(rcvPacket->getSrcID());
			...
			// Como recebi pacote de dados do nodo, zero seu contador no lost_association
			lost_association[rcvPacket->getSrcID()] = 0;
...
void Basic802154::handleAckPacket(Basic802154Packet * rcvPacket)
{
	...
	switch (currentPacket->getMac802154PacketType()) {
		...
		case MAC_802154_ASSOCIATE_PACKET: {			
			...
			// Testo se recebi um -1. Se for, fui negado a associação e devo entrar em baixissimo duty cycle
			if(associatedPAN == -1){
				trace() << "Denied Association by PAN!!";
				setMacState(MAC_STATE_SETUP);
				clearCurrentPacket("Success",true);
				//AQUI entrar em baixissimo duty cycle, dormindo imediatamente e programando para acordar,
				// obedecendo o baixissimo duty cycle definido na implementação
				//Primeiro, cancelando o timer de FRAME_START jã ativo
				if(getTimer(FRAME_START) != -1){
                			cancelTimer(FRAME_START);
					trace() << "FRAME_START ESTAVA ATIVO! CANCELADO!";
				}
				//programando a hora de acordar!!!
				trace() << "Vou dormir agora e so acordo daqui a: " << low_dc * baseSuperframeDuration * (1 << beaconOrder) * symbolLen;
				setTimer(LOW_DUTYCYCLE, low_dc * baseSuperframeDuration * (1 << beaconOrder) * symbolLen);
				//dormindo agora!!!
		...
		case MAC_802154_DATA_PACKET: {
			associatedPAN = rcvPacket->getPANid();
			//faço a verificacao se o PAN é -1, se for é pq nao tinha autorização de enviar pacote
			//Assim, devo me considerar nao associado e tentar se associar novamente
			if(rcvPacket->getPANid() == -1){
				trace() << "NAO TENHO AUTORIZACAO DE ENVIAR PACOTES";
				setMacState(MAC_STATE_SETUP); //redundante, pois quando ele for dormir, vai para status sleep
				clearCurrentPacket("Success",true);
				//AQUI entrar em baixissimo duty cycle, dormindo imediatamente e programando para acordar,
				// obedecendo o baixissimo duty cycle definido na implementação
				//Primeiro, cancelando o timer de FRAME_START jã ativo
...
void Basic802154::attemptTransmission(const char * descr)
{
	...
	if (macState == MAC_STATE_SLEEP || macState == MAC_STATE_SETUP || macState == MAC_STATE_LOW_DC) return;
...
// A function to get the beacon interval in seconds.
// ACTION: return the conversion of the beacon interval in seconds.
double Basic802154::getBeaconIntervalInSeconds() 
{
	// beaconInterval * symbolLen;
	return baseSuperframeDuration * (1 << beaconOrder) * (1 / (phyDataRate * 1000 / phyBitsPerSymbol));
}

// A function to get the superframe duration in seconds.
// ACTION: return the conversion of the superframe duration in seconds.
double Basic802154::getSuperframeDurationInSeconds()
{
	// frameInterval * symbolLen;
	return baseSuperframeDuration * (1 << frameOrder) * (1 / (phyDataRate * 1000 / phyBitsPerSymbol));
}
